...
#Placeholder in case I fuck up
placeholder <- phys_data
phys_data <- placeholder
...
phys_data <- phys_data %>% group_by(group,study,trial) %>% 
  mutate(
    ...
    # For each previous
    future_HR1 = lead(HR1,1),
    future_HR2 = lead(HR2,1),
    future_Resp1 = lead(Resp1,1),
    future_Resp2 = lead(Resp2,1),
    
    # For each change
    change_HR1 = (future_HR1 - HR1),
    change_HR2 = (future_HR2 - HR2),
    change_Resp1 = (future_Resp1 - Resp1),
    change_Resp2 = (future_Resp2 - Resp2))

# Make the data long, so we can analyze both participants at the same time 
## N.B. This is a bit tricky and you might have to do it in several steps

# Long format for HR_change_self
d_hr_change <- 
  gather(phys_data,
         participant, HR_change_self, #New variables
         change_HR1, change_HR2) %>% #Variables we already have
  
  select(#selecting relevant variables
    actual_time_min, HR_change_self, participant, study, group, trial, condition) %>% 
  
  mutate(#Unique ID for each participant
    participant = 
      parse_number(as.character(study))*1000 +
      parse_number(as.character(group))*10 + 
      parse_number(participant))


# Long format for HR_change_other
d_hr_change_other <- 
  gather(phys_data,
         participant, HR_change_other, #New variables
         change_HR1,change_HR2) %>% #Variables we already have
  
  select(#selecting relevant variables
    file, actual_time_min, HR_change_other, participant, study, group, trial, condition) %>% 
...
names(d_hr_change_other)[1] <- "participant"
names(d_hr_change_other)[4] <- "identifyer"

d_hr_change_other$participant <- ifelse(d_hr_change_other$identifyer == "change_HR2", d_hr_change_other$participant -1, +1)



# USE column bind !!!! to extract HR_change_other and put in the other data set

# Set the most interesting contrast e.g. by defining synchronous or conversation as the baselin
